    ...
    for (const [_key, user] of newUsers) {
      ...
      const member =
        message.guild.members
          .resolve(user)
          ?.roles?.cache?.find((r) => r.name === "Member") !== undefined;

      const councilMember =
        message.guild.members
          .resolve(user)
          ?.roles?.cache?.find((r) => r.name === "Council") !== undefined;
      const contributor =
        message.guild.members
          .resolve(user)
          ?.roles?.cache?.find((r) => r.name === "Contributor") !== undefined;
...
const reviewApplications = async (interaction) => {
  if (
    interaction.member.roles.cache.some((role) => role.name === "Council") ||
    ...
    for (var i = 0; i < allMessages.length; i++) {
      ...
      var isMember = await channel.guild.members
        .resolve(fetchedMessage.author)
